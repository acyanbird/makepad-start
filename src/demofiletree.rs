    ...
    pub fn load_file_tree(&mut self, tree_data: FileTreeData) {
        fn create_file_node(
            ...
            let name = parent_edge.as_ref().map_or_else(    
                || String::from("root"),    // If there is no parent edge, the name is "root".
                | edge | edge.name.clone(), // Otherwise, the name is the name of the parent edge.
            );
            let node = FileNode {
                parent_edge,
                name,
        ...
        match event{
            ...
            Event::Startup=>
            {
                fn get_directory_entries(path: &Path, with_data: bool) -> Result<Vec<DirectoryEntry>, FileError> {
                    ...
                    for entry in fs::read_dir(path).map_err( | error | FileError::Unknown(error.to_string())) ? {   // for ?, if error stop func
